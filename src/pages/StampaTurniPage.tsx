...
import { giorniDelMese } from '../lib/turniLogic'
...
export function StampaTurniPage() {
  ...
  // È la STESSA funzione usata dal pulsante «Scarica PDF Calendario» della pagina pubblica,
  // così le due strade producono esattamente lo stesso documento.
  const fogli = useMemo(() => costruisciFogliPdf({ fogliConTurni, giorni, turni, turnisti, festivoSet }),
    [fogliConTurni, giorni, turni, turnisti, festivoSet])
  ...
  /** Scarica il PDF: stessa funzione usata dal pulsante pubblico ⇒ documento identico. */
  async function scaricaPdf() {
    if (!fogli.length || pdfBusy) return
    setPdfBusy(true); setPdfErr(null)
    try {
      await scaricaPdfCalendario(fogli, { postazioneNome: postazione?.nome ?? '', mese, anno, festivoSet })
    } catch (e) {
      console.error('[Stampa] generazione PDF fallita:', e)
      setPdfErr('PDF non riuscito — usa «Stampa».')
        ...
        {fogli.map(({ foglio, righe, conRep }) => (
          <div key={foglio.id} className="foglio-stampa" style={{ marginBottom: 26 }}>
            <table style={{ borderCollapse: 'collapse', width: '100%' }}>
              ...
              <tbody>
                {righe.map(({ d, turno, nomi, rep }) => (
                  <tr key={`${isoDate(d)}|${turno.id}`}>
