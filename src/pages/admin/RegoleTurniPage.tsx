...
import { ChevronLeft, ChevronRight, ListChecks, AlertCircle, AlertTriangle, Plus, X, Infinity as InfinityIcon } from 'lucide-react'
  ...
  return (
    <div className="card p-3 flex flex-wrap items-center gap-x-4 gap-y-2">
      ...
      <label className="flex items-center gap-1.5 text-sm cursor-pointer">
        <input type="radio" checked={perSempre} onChange={() => onChange(null)} style={{ accentColor: '#476540' }} /><InfinityIcon size={14} /> Per sempre
      </label>
      <label className="flex items-center gap-1.5 text-sm cursor-pointer">
        <input type="radio" checked={!perSempre} onChange={() => onChange(`${ey}-${String(em).padStart(2, '0')}`)} style={{ accentColor: '#476540' }} /> Fino a
      </label>
      {!perSempre && (
...
export function RegoleTurniPage() {
  ...
  function cellaSlots(giorno: number, turno: TurnoSchema): (string | null)[] {
    return Array.from({ length: turno.n_turnisti }, (_, slot) => mappa.get(`${giorno}|${turno.id}|${slot}`) ?? null)
  }
  ...
  function handleDrop(giorno: number, turno: TurnoSchema) {
    ...
    setOverKey(null)
    if (!tid) return
    if (!turnoApplicabileGiorno(turno, giorno)) return
    ...
    setReg(giorno, turno.id, free, tid)
  }

  function cambiaMese(delta: number) { let m = mese + delta, a = anno; if (m < 1) { m = 12; a-- } else if (m > 12) { m = 1; a++ }; setMese(m); setAnno(a) }
  ...
  const Header = (
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1">
        ...
        <p className="text-sm text-stone-600">Trascina i turnisti dalla colonna sinistra nelle celle: chi c'è <strong>sempre</strong> in ogni turno della settimana.</p>
      </div>
  ...
  const avviso = (testo: ReactNode) => (
    <div className="max-w-4xl mx-auto p-6 space-y-4">{Header}<div className="card p-5 flex items-start gap-3"><AlertCircle className="shrink-0 mt-0.5" style={{ color: '#b45309' }} size={18} /><p className="text-sm text-stone-600">{testo}</p></div></div>
  )
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <p className="text-xs text-stone-500">
        Regole valide da <strong>{meseLabel(regoleVer.valido_da)}</strong>{regoleVer.valido_fino ? <> a <strong>{meseLabel(regoleVer.valido_fino)}</strong></> : <> in poi (per sempre)</>}. Caselle grigie = turno non previsto quel giorno.
      </p>
      ...
      <div ref={containerRef} className="flex gap-3 items-start"
        ...
        {/* Colonna sinistra: palette turnisti */}
        <aside className="w-40 sm:w-44 shrink-0 space-y-3">
          <div className="card p-2">
            ...
            <div className="flex flex-col gap-1.5">
              {gruppoTurnisti.length ? gruppoTurnisti.map(PaletteBadge) : <span className="text-xs text-stone-400 px-1">nessuno</span>}
            </div>
          </div>
          <div className="card p-2">
            <h3 className="text-[11px] font-bold uppercase tracking-wider px-1 mb-1.5" style={{ color: '#166534' }}>Esterni</h3>
            <div className="flex flex-col gap-1.5">
              {gruppoEsterni.length ? gruppoEsterni.map(PaletteBadge) : <span className="text-xs text-stone-400 px-1">nessuno</span>}
            </div>
          </div>
        </aside>

        {/* Colonna destra: griglia settimanale */}
              ...
              <tr>
                ...
                {schema.map(c => (
                  <th key={c.id} style={thStyle(false)}>
                    ...
                    <div style={{ fontSize: 10, fontWeight: 400, opacity: 0.85 }}>{c.ora_inizio}–{c.ora_fine} · {c.n_turnisti}</div>
                  </th>
                ))}
              </tr>
            </thead>
                        ...
                        <div className="flex flex-col gap-2 items-start">
                          {slots.map((tid, slot) => tid ? (
                            <span key={slot} className="relative rounded px-2 py-0.5 text-[11px] font-medium shadow-sm" style={{ background: coloreTurnista(tid).bg, color: coloreTurnista(tid).fg }}>
                              {nomeTurnista(tid)}
                              <button onClick={() => setReg(g.num, c.id, slot, null)} title="Togli"
      ...
      {/* Toast di avviso (controlli di consistenza) */}
      {warn && (
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none px-4" role="alert">
          <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl shadow-2xl pointer-events-auto"
            style={{ background: '#fef3c7', borderLeft: '5px solid #d97706', color: '#78350f', maxWidth: 460, animation: 'fadeSlideIn 180ms ease-out' }}>
