...
export function PostazioneProvider({ user, children }: { user: AuthUser | null; children: ReactNode }) {
  ...
  const { data: gestiteIds = [], isLoading: l2 } = useQuery<string[]>({
    queryKey: ['postazioni-supervisione', user?.id],
    queryFn: () => store.getPostazioniSupervisione(user!.id),
    enabled: !!user && user.isSupervisore && !user.tuttePostazioni,
  })

  // Postazioni visibili: admin o supervisore-di-tutte → tutte; supervisore → quelle
  // assegnate; turnista/esterno → la propria (non usano il selettore in admin).
  const postazioni = useMemo(() => {
    if (!user) return []
    if (user.livello === 'admin' || user.tuttePostazioni) return tutte
    if (user.isSupervisore) return tutte.filter(p => gestiteIds.includes(p.id))
  ...
  const value = useMemo<Ctx>(() => ({
    ...
    loading: !!user && (l1 || (user.isSupervisore && !user.tuttePostazioni && l2)),
